...
이 문서는 Perl에서 정규표현식의 이해, 작성, 사용을 위한 기본 설명서입니다.
또한 이 문서는 정규표현식 L<perlre> 문서를 보충하는 참고문서입니다.
정규표현식들은 C<m//>, C<s///>, C<qr//>의 내부 구성요소이고
C<split> 연산자이고  또한 이 문서는 L<perlop/"Regexp Quote-Like Operators">와
L<perlfunc/split>의 설명도 보충합니다.
...
Perl은 문자열 처리의 탁월함으로 매우 유명하고 정규표현식들은 이러한
펄의 명성을 뒷받침하는 요인들 중 하나입니다. 펄 정규표현식은 다른 컴퓨터
언어들은 알수없는 효율성과 유연함을 과시합니다. 당신은 정규표현식의
기초만 숙지하여도 놀랍도록 쉬운 텍스트처리가 가능할것입니다.
...
정규표현식은 무엇일까요? 정규표현식은 패턴을 표현하는 단순한 문자입니다.
요즘에 패턴은 일반적으로 사용됩니다. 웹페이지들을 찾기위한 검색엔진에서
타이핑되는 패턴들과 C<ls *.txt> 또는 C<dir *.*>처럼 디렉토리안의 파일들을
나열하기 위해 사용되어지는 패턴들이 그 예입니다. 펄에서, 패턴은 문자열을
찾거나, 문자열의 원하는 부분을 추출하거나, 검색과 치환 연산들을 위해 사용
되는 정규표현식으로 기술됩니다.
...
정규표현식은 추상적이고 이해하기 어렵다는 부당한 평가를 받습니다.
정규표현식은 조건과 loop와 같은 간단한 개념으로 구성되어 있고,
펄 언어 안에 있는 C<if> 조건문과 C<while> loop와 비교했을때 이러한 개념을
이해하는 것은 어려운 것이 아닙니다. 사실, 정규식을 배우는데 주된 문제점은
저런 개념을 표현하기 위해 사용된 간결한 표기법의 사용법을 익히는것입니다.
#!# 마지막문장은 해석이 좀;
...
이 설명서는 정규표현식 개념과 함께 표기법을 한번에 하나씩 많은 예제들과 함께 
설명함으로서 학습곡선을 완만하게 합니다. 이 설명서의 첫부분은 정규표현식 개념을
위하여 가장 간단한 단어 검색들부터 진행할것입니다. 만약 당신이 첫부분을
익힌다면 당신이 하고자하는 일의 98%를 해결할수있는 모든 도구를 갖게 될것입니다.
이 설명서의 두번째 부분은 기초들을 편안하게 하고 더욱 강력한 도구를 원하는
이들을 위한 것입니다. 더욱 진보된 정규표현식 연자사들을 논의하고 5.6.0의
가장 최근의 cutting edge innovations을 소개합니다.
...
참고: 시간을 절약하기 위해, 'regular expression'은 자주 regexp 또는 regex 로
짧게 표현합니다. regexp 가 regex 보다 좀더 자연스러운 짧은 표현이지만, 발음상
조금 더 어렵습니다. Perl pod 문서는 regexp, regex 모두 고르게 사용합니다;
Perl에서는 이것을 짧게 표현하기 위한 하나 이상의 방법이 있습니다.
이 설명서에서 우리는 regexp 를 사용할 것입니다.

#!# 번역은 정규표현식 대신 정규식으로 합니다.
...
가장 간단한 정규식은 간단하게 하나의 단어나 좀더 일반적으로 문자들의 연속
(문자열)입니다. 하나의 단어로 이루어진 정규식은 그 단어를 포함하고 있는
어떠한 문자열과도 매치합니다:
...
이 Perl 문장은 모두 무엇일까요? C<"Hello World"> 는 단순히 더블쿼터로 둘러쌓인
문자열입니다. C<World> 는 정규식이며 C</World/>를 감싸고있는 C<//> 는 펄에게
매치를 위한 문자열 검색을 펄에게 알려줍니다.
# sj
C<=~> 식은 문자열과 같이 정규식 매치(regexp match)로 결과에 따라 match
가 되었을경우 true, 안되었을경우 false를 생성합니다. 이 경우에는
C<World>가 C<"Hello World">의 두번째 단어와 매치되었기때문에 식은 true
인것입니다. 이런식은 다음과같은 경우에 쓰면 좋습니다:
# sj
...
같은 주제로 유용한 변형들이 있습니다. C<!~> 연산자를 사용함으로서 매칭의 의미는
정반대가 되어집니다.
...
특수한 기본변수 C<$_>에 대해 매칭을 원한다면, C<$_ =~>부분은 생략가능합니다.
...
그리고 마지막으로, 매치를 위한 기본구분자 C<//>는 C<'m'>을 앞부분에 넣어
마음대로 구분자를 변경할 수 있습니다.
...
C</World/>, C<m!World!>, 그리고 C<m{World}> 모두 같은 것을 표현한 것입니다.
예를 들어, 쿼터 (C<">)는 구분자로 사용되어졌을때, 다음의 슬래쉬 (C<'/'>)는
일반적인 문자가 되고, 이는 정규식안에서 문제없이 사용가능합니다.
...
C<"Hello World">를 정규식들이 매치할때 어떻게 다른지 생각해보자.
...
정규식은 대소문자를 구별하기 때문에 첫번째 정규식 C<world>은 매치되지
않습니다. 두번째 정규식은 부분문자열 S<C<'o W'>>이 문자열 S<C<"Hello World">>
안에서 발견되었기 때문에 두번째 정규식은 매치됩니다. 공백문자 ' '은
정규식안에서 어떤 다른문자와 같이 간주되고 이 경우는 매치하기 위해
필요합니다. 공백문자가 빠졌기 때문에 세번째 정규식 C<'oW'>은 매치되지
않습니다. 마지막의 공백이 있기 때문에 네번째 정규식 C<'World '>은
매치되지 않습니다. but not at the end of the string. (해석이 애매함)
문자열이 마지막, 즉 공백이 없다면 참입니다.
참이 되기 위한 문장을 위해서는 정규식은 <exactly> <정확한> 순서의 문자열의
일부분을 매치 되어진다는 것을 알아야 합니다.
...
문자열 안에서 한군데 이상에서 정규식이 매치된다면, 언제나 펄은
문자열의 가장 먼저 매치되는곳에서 매치될 것입니다.
...
문자가 매치되어지 것과 관련하여, 당신이 숙지할 필요가 있는 항목들이
몇가지 더 있습니다. 그것들 중에 첫번째는 매치를 위해서 문자들만 사용되어지는
것이 아니라는것입니다. 정규식 표기법에는 <metacharacters> 라고 불리는, 
몇가지 문자들이 지정되어 있습니다.
...
이들의 각각의 의미는 설명서의 나머지 부분에서 설명할 것입니다, 하지만
지금 당장은, 메타문자는 메타문자 앞에 역슬러시를 넣음으로서
매치되어지는것을 숙지하는것은 중요합니다.
...
마지막 정규식에서, 정규식의 경계를 구분하기 위해 C<'/'>앞에
역슬러시가 사용되었습니다. 이것은 LTS(leaning toothpick syndrome)를
유발합니다. 하지만, 이경우는 종종 정규식 구분자를 변경하는것이
한층 읽기 쉬워집니다.
...
백슬러시 문자 C<'\'>는 그 자체로 메타문자이고 백슬러시를 표현하기
위해서는 백슬러시가 필요합니다.
...
메타문자들과 관련하여, I<escape sequences>를 대신하고 이와 동등한 의미를
가진 출력가능한 문자가 아닌 여러가지 ASCII 문자들이 있습니다.
tab을 위한 C<\t>, newline을 위한 C<\n>, carriage return을 위한 C<\r>
그리고 bell을 위한 C<\a>이 일반적인 예들입니다. 만약 당신의 문자열이
임의의 바이트의 순서로 보는것이 더 나은 생각이라면,
8진수 escape sequence, C<\x1B>, 또는 16진수 escape sequence, C<\x1B>
같은 예는 당신의 바이트를 더욱 자연스럽게 표현합니다.
여기에 escape의 몇가지 예들이 있습니다.
...
당신이 펄을 조금만 사용하여도, escape sequence들에 대한 이런 이야기는
매우 친숙하게(familiar) 보여질것입니다. 유사한 escape sequence들은
더블쿼터로 쌓여진 문자열들 안에서 사용되고 사실 펄의 정규식들은 더블쿼터로
둘러쌓인 문자열들로 취급됩니다. 이것은 정규식들 안에서 변수들이 잘 사용
되어질수 있음을 의미합니다. 쉽게 말해서 더블쿼터로 둘러쌓인 문자열들 즉,
정규식안의 변수들의 값들은 매칭의 목적를 위해 정규식이 평가되기 전에
치환되어질것입니다.
...
좋습니다. 위의 내용을 알고 있다면, 당신은 이미 당신이 생각하는 어떠한
리터럴 문자열 정규식에 대한 검색들을 실행할 수 있습니다.
여기 유닉스 grep 프로그램의 I<very simple> emulation 이 있습니다.
...
이 프로그램은 이해하기 쉽습니다. C<#!/usr/bin/perl>은 쉘에서 펄 프로그램을
호출하기 위한 일반적인 관례입니다. S<C<$regexp = shift;>>은 파일들을 다루기
위한 커맨드라인 인자들의 나머지는 남기고, 정규식에 사용할 첫번째 커맨드라인
인자를 저장합니다. S<C<< while (<>) >>>은 모든 파일들의 모든 라인들을
반복합니다. 각 라인에서, S<C<print if /$regexp/;>>은 정규식이 그 라인에서
매치된다면 라인을 출력합니다. 이 라인 안에서, C<print>와 C</$regexp/> 모두
기본변수 C<$_>를 암묵적으로 사용합니다.
...
위 정규식들 모두는, 문자열안의 어느곳에서나 정규식이 매치되었다면,
그것은 매치로 간주되어졌습니다. 때로는, 하지만, 우리는 매치를 시도하는
문자열안에서 I<where>를 기술하기를 원합니다. 이를 위해서는, 우리는
I<anchor> 메타케릭터들 C<^>와 C<$>를 사용할 것입니다. 앵커 C<^>는 문자열의
시작에서 매치를 의미하고 앵커 C<$>는 문자열의 끝에서 매치, 또는 문자열의
끝 앞에 있는 newline 전에서 매치를 의미합니다.
...
C<^>은 C<keeper>이 오직 문자열의 시작에서 매치되기를 강제하지만
C<housekeeper>은 keeper 문자열이 중간에서 시작되기 때문에 두번째 정규식은
매치되지 않습니다. 세번째 정규식은 매치됩니다. C<$>은 C<keeper>을 오직
문자열의 마지막에서 매치되기를 강제했습니다.
...
C<^>와 C<$> 둘다 같이 사용되어졌을때, 정규식은 문자열의 시작과 끝 둘다
매치되어야합니다. 예를 들면, 정규식은 문자열 전체와 매치합니다. 잘생각해보자.
...
C<keep>보다 더 많은 문자열을 가지기 때문에 첫번째 정규식은 매치하지 않는다.
두번째 정규식은 정확하게 문자열에 일치하고, 매치된다. 정규식에서 C<^>와
C<$> 모두를 사용하는것은 문자열 전부와 매치를 강요하므로,
# sj
당신에게 어떤 문자열이 매치되고 안되는지에 대한 풀 컨드롤을 줍니다.
만약에 당신이 bert라고만 쓰여진 사람을 찾고 싶다면:
# sj
...
물론, 리터럴 문자열의 경우에서는, 문자열 비교 S<C<$string eq 'bert'>>를
사용하는 것이 더 쉽고 더욱 효과적입니다. C<^...$> 정규식은 뒤에서 설명하는
더욱 강력한 정규식 안에 서술될때 정말로 유용합니다.
...
위에서 설명한 리터럴 문자열 정규식들로 이미 많은 것을 할수 있을지라도,
우리는 단지 정규표현식의 맛만 본 것입니다. 이번과 다음 절에서 우리는
단지 한개 문자 sequence가 아닌 그들의 I<whole class>를 표현하는
정규식 개념(메타문자 표기법과 관련된)을 소개할 것입니다.
...
그런 개념들 중 하나가 I<character class>입니다. 문자클래스는 정규식
안에서 특정 부분을 매치하기 위한 단지 하나의 문자라기보다는 문자들의
집합을 의미합니다. 문자클래스들은 매치되기를 원하는 문자들의 집합과
함께, 괄호 C<[...]>를 사용하여 표기합니다.
여기 몇가지 예들이 있습니다:
...
마지막 문장에서는, 이 클래스 안에서 첫번째 문자가 C<'c'>일지라도 C<'a'>가
매치됩니다. 왜냐하면 문자열의 첫번째 문자의 위치가 정규식이 매치할수있는
가장 빠른 지점이기 때문입니다.
...
이 정규식은 대소문자와 구분없이 매치하는 일반적인 작업을 보여줍니다.
펄은 매치의 마지막 부분에 C<'i'>를 간단히 추가함으로서 모든 괄호들을
서술하지 않는 방법을 제공합니다. 그래서 C</[yY][eE][sS]/;>은 C</yes/i;>로
재작성 가능합니다. C<'i'>는 대소문자를 구분하지 않음을 의미하고 이것은
매칭 연산자 I<modifier>의 한 가지 예입니다. 이 설명서에서 우리는 나중에
다른 변경자들을 만나게 될것입니다.
...
이전 절에서, 우리는 그들자신을 표현하는 평범한 문자들과 그들자신을 표현하기
위해 C<\>가 필요한 특수한 문자들을 보았습니다. 이것은 문자클래스에서도
동일하지만, 문자 클래스 안쪽에는 문자 클래스 바깥에 있을때와 다른 의미를 갖는
평범하고, 특수한 문자들의 집합들이 있습니다. 그런 특수한 문자들은 C<-]\^$>
입니다(어떤 문자가 패턴구분자가 될지 모르겠지만 그것 역시 포함됨).
C<]>은 문자클래스의 끝을 표기하기때문에 특별합니다. C<\>은 위에서처럼
escape sequences로 사용되어지므로 특별합니다. 여기에 특별한 문자들 C<]$\>을
처리하는 방법이 있습니다:
...
마지막 두 문자은 조금 신중해야합니다. C<[\$x]>에서, 백슬러시는 달러 기호를
보호했기 때문에 문자 클래스는 두개의 멤버 C<$>와 C<x>를 가집니다.
C<[\\$x]>에서는, 백슬러시는 보호되어지고 그래서 C<$x>는 변수로 취급되어
더블쿼터 방식으로 치환되어집니다.
...
특수변수 C<'-'>은 문자 클래스들내에서 범위 연산자로 작동하고, 그래서
서로 이웃관계에 있는(contiguous) 문자들의 집합은 범위로 작성 가능합니다.
범위연산자를 사용하면, 꼴사나운 C<[0123456789]>와 C<[abc...xyz]>는
세련된 C<[0-9]>와 C<[a-z]>로 표현할 수 있습니다. 몇가지 예를 들면
...
만약 C<'-'>이 문자 클래스안에서 처음과 마지막 문자라면, 평범한 문자로 취급
됩니다; C<[-ab]>, C<[ab-]> 그리고 C<[a\-b]>은 모두 같은 의미입니다.
...
문자 클래스의 첫번째 지점에서 특수문자 C<^>은 괄호들 안의 문자가 아닌
그 어떤 문자와도 매치되는 I<negated character class>를 의미합니다.
...
이제는 C<[0-9]>조차도 매번 서술하는것도 귀찮고, 키보드 타이핑을 적게하고
더욱 읽기쉬운 정규식들의 작성을 위해, 펄은 일반적인 문자클래스들을 위한
아래에 보이는 여러가지 단축형들을 가지고 있습니다. 유니코드의 도입 이후로,
이런 문자 클래스들은 ISO 8859-1 범위의 소수의 문자들보다 더 많이 매치됩니다.
...
C<\d\s\w\D\S\W> 축약어들은 문자 클래스들의 안과 밖 모두 사용 가능합니다.
여기 몇가지 사용법이 있습니다:
...
마침표는 메타문자이므로, 평범한 문자 마침표로 매치되려면 escape가 필요합니다.
예를 들어, C<\d>와 C<\w>는 문자들의 집합들이므로, C<[\D\W]>을 C<[^\d\w]>로
평가하는 것은 올바르지 않습니다; 사실 C<[^\d\w]>은 C<[^\w]>와 같고, 이것은
C<\W]>와 같습니다. 드모르간의 법칙을 생각해보세요.
...
기본 정규식들에서 유용한 앵커는 I<word anchor> C<\b>입니다.
이것은 단어와 비단어 즉 C<\w\W> 또는 C<\W\w> 사이에 boundary를 매치합니다.
...
마지막 예제를 참고하자면, 문자열의 마지막은 단어 boundary로 간주되었습니다.
...
당신은 C<'.'>이 왜 모든 문자가 아닌 C<"\n">를 제외한 모든 것과 매치되는지
이상하게 여길수도 있습니다. 그 이유중 하나는 뉴라인 문자들을 무시하기를
원하는 라인들을 매치한다는 것입니다. 예를 들어, 문자열 C<"\n">이 한개의
라인의 전부라면, 우리는 그 라인이 비었다고 생각하고 싶어합니다. 그러면
...
우리는 보통 하나의 라인에서 문자들을 카운트하고 매치할때 뉴라인들을 무시하는
것을 원하기 때문에 이런 행동은 편리합니다. 때로는, 하지만, we want to keep
track of newlines. 우리는 단순히 문자열의 처음과 끝보다는 문자열 안쪽에 있는
라인들의 처음과 끝에서 앵커로서의 C<^>와 C<$>을 더욱 원할수도 있습니다.
펄에서는 C<//s>와 C<//m> 변경자들을 이용해서 뉴라인들을 무시하거나 적용시킬수
있습니다. C<//s>와 C<//m>은 한라인과 멀티라인을 의미하며 어떤 문자열을 하나의
연속된 문자열로 취급할것인가, 또는 라인들의 집합으로 취급할것인가를 정의합니다.
두 변경자는 정규식이 어떻게 해석되어 것인가에 대해 두가지 영향을 미칩니다:
1) C<'.'> 문자 클래스가 어떻게 정의되어지는것인가, 그리고 2) 앵커들 C<^>과
C<$>이 어디에서 매치될것인가. 여기에 4가지 가능한 조합이 있습니다.
...
변경자가 없는 경우 (//): 기본 행동. C<'.'>은 C<"\n">을 제외한 모든 문자와
매치됩니다. C<^>은 오직 문자열의 시작에서 매치되고 C<$>은 오직 문자열의
끝에서 매치되거나 문자열 끝에 있는 뉴라인 전에서만 매치됩니다.
...
s 변경자 (//s): 문자열을 하나의 긴 라인으로 취급합니다. C<'.'>은
C<"\n">을 포함한 모든 문자와 매치됩니다. C<^>은 오직 문자열의 시작에서만
매치되고 C<$>은 오직 문자열의 끝에서 매치되거나 문자열 끝에 있는 뉴라인
전에서만 매치됩니다.
...
m 변경자 (//m): 여러줄들의 집합으로 문자열을 취급합니다. C<'.'>은 C<"\">을
제외한 모든 문자와 매치됩니다. C<^>와 C<$>는 문자열 안의 I<any> 라인의
시작과 끝에서 매치 가능합니다.
...
s와 m 변경자 둘다 사용 (//sm): 여러줄들을 인식하지만, 문자열을 하나의 긴
라인으로 취급합니다. C<'.'>은 C<"\n">을 포함한 모든 문자와 매치됩니다.
하지만, C<^>와 C<$>은 문자열 안의 I<any> 라인의 시작과 끝에서 매치 가능합니다.
...
대부분, C<//s>와 C<//m>은 가끔은 매우 유용하지만 기본적인 동작만으로
충분합니다. 만약 C<//m>이 사용되었다면, 
