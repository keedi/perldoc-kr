...
Ext.grid.dummyData = [
    ...
    [
      ...
      '역사가 긴 Perl 특성상 과거의 오래된 자료와 서적이 많아 사용자들이 철 지난 자료를 기준으로 공부하는 경우가 흔하다. Perl이 그간 많은 버전업을 거치며 발전해 왔음에도 대부분의 사용자들은 새로운 특징을 잘 모르고 과거 방식으로 Perl 코드를 작성한다.  Perl의 최신 경향과 스타일을 살펴봄으로써 이제는 Old Perl에서 탈출하도록 하자!'
    ],
    [
      'Perl Basic Skill Up',
      '전종필',
    ...
    [
      ...
      '14:00 ~ 14:20',
      '1. 정규 발표 1부 (Regular Talk, Part #1)',
      'Perl을 이용한 프로토타입 개발에 대하여 살펴본다. 위치예측은 인텔리전트 빌딩이나 스마트 홈 네트워크를 구축하기 위한 기반 기술 중 하나이다. 위치예측에 관한 많은 연구들이 수행되어 왔지만 대부분의 연구는 단일 이동체의 위치 예측에 중점을 두고 있다. 보다 실용적인 연구를 위하여 다수의 이동체가 동시에 이동하는 경우를 고려한 알고리즘을 고안하였다. 알고리즘의 실효성을 검증하기 위하여 단순히 컴퓨터 시뮬레이션만 하는 것이 아니라 실제 테스트베드를 구축하여 실험을 해야 했다. 센서네트워크를 구축하여 이동체의 이동 정보를 인식하고 실시간으로 정보수집서버로 전달하여 위치예측을 수행한다. Perl을 이용하여 빠르게 알고리즘의 프로토타입을 구현하여 테스트베드를 구축할 수 있었다.'
    ],
    [
      'Web2RSS with Perl',
      '김기석',
      '14:30 ~ 14:50',
      '1. 정규 발표 1부 (Regular Talk, Part #1)',
      '자신이 자주 방문하는 웹페이지가 Feed를 제공하지 않는 경우를 종종 경험했을 것이다. 그런 Feed를 제공하지 않는 웹페이지의 변경 사항을  Perl을 이용하여 RSS Feed로 만들어 본다.  그 과정에서 Perl의 정규표현식으로 문서의 원하는 부분을 가져 오기, LWP모듈로 웹페이지 가져오기,  DBM해시를 이용해서 정보를 저장하기 등의 방법을 알아본다.'
    ],
    [
    ...
    [
      'Perl, Business and Money',
      '김민수',
      '20:05 ~ 20:10',
      '4. 짧은 발표 (Lightening Talk)',
      '실제 Perl이 산업현장에서 어떻게 쓰이고 회사는 어떻게 Perl로 비지니스에 도움이 되어 돈을 벌게 될수 있는지를 알아본다. 세계에서 제일 큰 시장을 가지고 있는 미국과 일본의 사례를 살펴보며, 국내에서 적용할 수 있는 방법을 제시한다. 엔지니어뿐만 아니라 누구나 쉽게 이해할 수 있도록 기술 용어는 배제하고 sales/business의 측면에서 Perl을 소개한다.'
    ],
    [
    ...
    ['Epilogue','김도형','20:40 ~ 21:00', '9. 정리하며 (Epilogue)','정리하며...']
];
