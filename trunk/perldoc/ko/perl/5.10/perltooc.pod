...
오브젝트 클래스를 디자인 할때 당신은 가끔 그 클래스의 오브젝트들이 공통으로
공유되는 값을 원할때와 대면하게 될것이다.
...
the class itself.
그러한 I<클래스 속성들>은 클래스 전체에 대해 전역변수와 같이 행동하는데
프로그램상의 전역이라고 보기보다는 클래스에 한해서라고 볼수 있겠다.

Here are a few examples where class attributes might come in handy:
클래스 속성들이 유용 할때의 예를 몇가지 들어 보도록 하겠다.
...
클래스를 이용해 마지막으로 생성된 오브젝트를 접근하기 위해, 아니면 가장 많이
접근된 오브젝트나, 모든 오브젝트에 대한 리스트를 가져오기 위해.


=back

Unlike a true global, class attributes should not be accessed directly.
본래의 전역변수와는 달리, 클래스 속성들 직접적으로 접근되어선 안된다.
...
대신에 클래스 속성들의 상태를 검사를 하거나, 혹은 변경하기 위해선 I<클래스
메소드>의 중개를 통해서만 이루어 져야 한다. 클래스 속성 접근자들은 본질적으로나
기능적으로 오브젝트의 인스탄스 속성들의 상태를 바꿀때 사용하는 접근자와
같다. 클래스 속성자들은 인테페이스 (interface) 와 구현 (implementation) 사이에
명백한 방화벽을 제공한다.
...
당신은 클래스 속성들의 접근을 클래스 이름이나 해당 클래스의 오브젝트들을 통해서
허용해야 한다. 만약에 우리가 $as_object가 Some_Class라는 타입이라고 가정하고
&Some_Class::population_count가 클래스 속성을 접근하는 메소드라면 그러면 이
두개의 함수 호출은 가능해야 하며 거의 같아야만 한다.
...
질문이 있다면, 당신은 어디에 상태값을 저장할것이면 어떤 메소드를 이용해 그
값들을 접근할 것인가? 이러한것들을 정적인 데이타 멤버라고 부르는 좀더 제약적인
C++ 언어와는 다르게, 펄은 클래스 속성을 선언하기 위해 따로 문법을 제공하지
않으며 인스탄스 속성을 정의할때 사용하는 문법 이상은 지원하지 않는다. 개발자에게
펄은 강력하면서도 유연성있는 폭 넓은 기능들을 제공하는데 이 각 기능들은 특정
상황에 요구되는 곳에 세심하게 만들어졌다.
...
펄에게 있어 클래스는 보통 모듈과 같이 구현될수 있다. 모듈은 서로 보완을 해주는
두개의 기능으로 나눌수가 있겠는데: 바깥세상과의 인터페이싱을 위한 패키지와
프라이버시를 위한 파일내의 렉시칼 영역. 이 두개의 메카니즘을 이용하여 클래스
속성들이 구현될수 있겠다. 이로 인해 당신은 클래스 속성들을 패키지 변수로 넣던지
렉시칼 변수로 넣을지 결정할수 있겠다.
...
# TODO: 오역이라 재해석이 필요함 context를 확실히 하고 번역 시도.
그것들뿐만 아니라 결정할것들이 있다. 만약 패키지 변수를 사용 하기로 선택했다면,
상속을 무시하던지 민감하게 생각해서 클래스 속성 접근자 메소들를 만들수
있겠다. 만약 렉시컬 변수를 선택했다면 전체 파일 영역에서 선출해서 변수들을
접근할수 있는 권한을 줄수 있으며 아니면 메소드들이 직접 속성들을 접근 할수 있는 부분을
확실히 제한을 두어 구현할수 있겠다.


=head1 Class Data in a Can
=head1 캔안에 들어 있는 클래스 데이타
...
어려운 문제를 푸는데에 있어 가장 쉬운 방법이 있다면 그건 다른 사람에게 그 문제를
풀게끔 나둘수 있겠다. 이경우, Class::Data::Inheritance (당신에게 가까운 CPAN에
있습니다)는 클래스 데이타에 관한 문제를 클로우저(Closure)를 이용하여 바로 따서
사용할수 있는 해결법을 제공한다.

=head1 Class Data as Package Variables
=head1 패키지 변수로 사용하는 클래스 데이타
...
to mediate access.
펄에서의 클래스는 결국 패키지나 마찬가지라서, 패키지 변수를 이용하여 클래스
속성을 담는 방법이 가장 자연스러운거 같다. 이를 통해 각 클래스들이 간단하게
자기들만의 클래스 속성을 가질수 있는거 같다. Some_Class라는 클래스를 가지고
있다고 가정하자 이 클래스는 파일 전체 내에서 전역변수로 사용할수 있는 여러
속성들을 필요로 한다. 가장 쉬운 방법으로 할수 있는 방법은 $Some_Class::CData1나
$Some_Class::CData2같은 패키지 변수들을 이용해 속성값들을 줄수 있겠다. 
우리는 외부에서 이러한 데이타들을 직접적으로 접근하는 것을 별로 원치 않을 것이다,
그래서 데이타 접급을 중계해주는 메소드들을 제공해 주도록 한다.
...
아래에 있는 접근자 메소드(accessor methods)들을 보면, 우선 첫번째 인자는
무시하도록 하자--메소드 실행시 화살표의 왼쪽 부분에 위치하는 클래스 이름이나
오브젝트 참조값(object reference)들을 말이다.
...
이 테크닉은 가장 명료하고도 초심자 펄 프로그래머 한테도 매우 무난한 방법이
되겠다. 패키지 변수 이름 전체를 명시해줌으로써 코드를 읽는데 있어 바로 눈에
띄일것이다. 안타깝게도 만약 변수들중에 오타를 내면 매우 잡기 힘든 에러를
초래하게 될것이며 하드코드된 클래스 이름들을 보고 꽤 당황하게 될것이다.
...
이 문제들은 쉽게 고쳐질수 문제들이 되겠다. 간단히 use strict pragma를 더해주고
미리(선행으로?) 패키지 변수들을 선언해 줌으로써 말이다. ( 5.6에서 새롭게 our
연산자가 제공 되며, 렉시칼 영역에서 사용되는 my 연산자와 같이 패키지 전역에서
사용할수 있겠다.)
...
다른 전역 변수들과 마찬가지로, 여러 프로그래머중에는 패키지변수 이름의 시작을
대문자로 주는것을 선호하기도 한다. 이를 통해 어느정도의 명료함을 주지만, 더이상
패키지 변수 로서의 자격을 상실해 코드를 읽을시 의미를 잃어 버릴수도 있겠다. 이런
문제들은 예제에 사용된 것들보다 더 적절한 이름들을 선택함으로 쉽게 해결할수
있겠다.
...
얼마 안있어 아무생각없이 열거된 인스탄스(intance)속성들의
접근자(accessor) 메소드들은 짜증을 키울것이며 또한 클래스 데이타의 접근자
메소드들에 대한 리스팅을 할때 그런 반복성으로 인해 성질이 나기
시작할것이다. 이런식의 되풀이는 프로그래머로써의 주된 장점에 반대되는
것이다: 즉 게으름인데, 모든 프로그래머들은 선천적으로 가능하면 중복된 코드를
피하는것을 볼수 있겠다.


Here's what to do.  First, make just one hash to hold all class attributes.
여기에 해결방법이 있다.  첫번째로 해쉬 하나를 생성하여 모든 클래스 속성들을 담는다.
...
여러 클로우저(closure)의 사용과 패키지 심볼 테이블에 직접 접근(direct access)을
통해, 각 %ClassData해쉬에 있는 키를 위해 접근자 메소드들을 클론을 한다. 이 각
메소드들을 통해 특정 클래수 속성 값을 가져오거나 저장할수 있겠다.
...
사실 &AUTOLOAD를 도입하여 해답을 구사할수 있겠지만, 이 방식으로는
만족성을 충당할수 없는듯하다. 작성된 함수는 값이 클래수 속성인지 오브젝트
속성인지 구별해야 하며; 상속을 방해 할수있으며; 섬멸자(DESTROY)를 조심히
다루어야 한다. 주로 이정도 어려운 구현방식은 요구되지않으며 여기에선 특히
더욱 그렇다.
...
여기서 왜 루프 안에는 strict ref를 무효화 했는지 궁금해 할껏이다. 우리는
여기서 심볼 테이블인 
