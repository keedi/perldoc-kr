...
오브젝트 클래스를 디자인 할때 당신은 가끔 그 클래스의 오브젝트들이 공통으로 공유
할수 있는 상태값을 원할때와 부딪히게 될것이다.
...
프로그램상의 전역이라고 보기보다는 클래스 속성들은 클래스 자체에서만 *효과*가
있다.
...
그렇지 않으면 클래스 속성들의 상태를 검사를 하거나, 혹은 바꾸거나, I<클래스
메소드>의 중개를 통해 이루어 져야 한다. 클래스 속성 접근자들은 본질적으로나
...
C++ 언어와는 다르게, 펄은 클래스 속성을 선언하기 위해 인스탄스 속성을 정의하는
문법 이상의 기능을 지원하지 않는다. 개발자에게 펄은 강력하면서도 그렇지만
유연성있는 폭 넓은 기능들을 제공하는데 이 각 기능들은 특정 상황에 요구되는 곳에
독특하면서 정성들여 만들어졌다.
...
풀도록 나둘수 있겠다. 이경우, Class::Data::Inheritance (당신에게 가까운
CPAN에 있습니다)는 클래스 데이타 문제를 클로져를 이용하여 바로 따서 사용할수
있는 해결법을 제공한다.
...
$Some_Class::CData2같은 패키지 변수들을 이용해 속성값들을 줄수 있겠다. 확실한게
있다면 우린 이 데이타를 외부에서 직접 에세스 하는 것을 원치 않을 것이다, 그래서
데이타 에세스를 중간에서 대신 담당하는 메소드들을 제공한다.
...
# TODO: context가 확실치 않아 오역으로 남겨짐
아래에 있는 접근자 메소드들을 보면, 우선 첫번째 인자는 무시하도록 하자--메소드
실행시 거시서부터 왼쪽방향의 화살표 즉 클래스 이름이거나 오브젝트 참조값인.
...
되겠다. 패키지 변수 이름 전체를 명시해줌으로써 코드를 읽는데 있어 명료하게 눈에
뛰일것이다. 안타깝게도 만약 이변수들중에 오타를 내면 매우 잡기 힘든 에러를
초래하게 될것이다.  그리고 하드코드된 클래스 이름들을 보고 꽤 당황하게 될것이다.
...
이 두문제들은 손쉽게 고쳐질수 있겠다. 간단히 use strict pragma를 더해주고
...
다른 전역 변수들과 마찬가지로, 프로그래머중에는 패키지변수 이름의 시작을
대문자로 주는것을 선호한다. 어느정도는 명료함을 주지만, 더이상 패키지 변수
이름으로서 적절하지 않아 코드를 읽을시 의미를 잃어 버릴수도 있겠다. 이런
문제들은 여기서 사용된 것들보다 더 적절한 이름들을 선택함으로 쉽게 해결할수
있겠다.
