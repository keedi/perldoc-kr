...
아래의 이름은 Perl 에서는 특별한 의미를 가집니다.
기호적인 이름의 많은 기억법이 있지만, 셸에서의 유추가 가능합니다. 
그래도 긴 이름을 사용하고 싶은 경우에는 
...
라고 프로그램의 맨 처음에 써주세요.
이것은 모든 짧은 이름의 별명으로써
최근 패키지에서의 긴 이름을 붙일 수 있습니다.
B<awk> 에서 가지고 온 중간적인 이름을 가지고 있는 것도 있습니다.
일반적으로 제일 좋은 방법은:
...
현재 선택된 파일핸들에 
존재하는 변수인 경우에는 대신에 IO::Handle 오브젝트에 관한 
오브젝트메소드를 호출해서 설정할 수 있습니다만, 
일반적인 내장변수보다는 효율이 떨어집니다.
(아래의 요약에서는 HANDLE 이라는 단어를 포함하고 있습니다).
우선 맨처음에 반드시
...
각각의 메소드는 IO::Handle 속성의 옛 값을 반환합니다.
메소드는 각각 EXPR 을 얻을 수 있고, 설정한 경우에는 
문제의 IO::Handle 속성의 새로운 값을 지정하게 됩니다.
설정한 경우에는 많은 메소드에서는 현재의 값에 대해서 아무것도 하지 않습니다만,
autoflush() 에서는 1을 지정한 것으로 합니다.
...
IO::Handle 클래스를 읽는 것은 비용이 높은 조작이기에, 
일반적인 내장함수의 사용법을 기억해둬야 합니다.
...
이것은 변수의 안에서는 "read-only" 로 취급되는 것도 있습니다.
즉, 그런 변수에 대해, 직접 리퍼런스를 건ㄷ리든 간접적으로 하든 
대입을 수행하려고 한 경우에는 실행시에 예외처리가 기동됩니다.
...
아래는 어떻게 자신의 코드가 무너지는 가하는 예제입니다:
...
아마도 이 코드는 아래처럼 표시되는 것을 기대하고 있습니다:
...
왜 그럴까요?
nasty_break() 는 C<$_> 을 로컬화하기 전에 변경하기 때문입니다.
수정하기에는 local()  을 추가합니다.
...
아래의 리스트는 우선 스칼라 변수, 그리고 배열, 해쉬의 순으로 나열합니다.
...
기본ê의 입력과 패턴 검색의 스페이스, 아래의 2가지는 같은 값입니다.
...
Perl 이 (당신이 사용하고 싶지 않아도) $_ 을 가정ㅏ는 경우가 몇가지 있습니다:
...
여러가지 단항함수. ord() 나 int() 또는 C<-t> 이외의 모든 
파일테스트(C<-f>, C<-d>) 등, C<-t> 의 기본값은 STDIN 입니다.
