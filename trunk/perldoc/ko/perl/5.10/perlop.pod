...
Perl 에서의 연산자의 우선순위와 결합성은 적잖은 부분에서 수학과 비슷합니다.
...
I<연산자와 우선순위> 라는 것은 다른 연산자보다 먼저 평가받는 연산자가 있다는 것입니다.
예를들면, C<2 + 4 * 5>의 경우 곱셈이 높은 우선순위를 가지고있기 때문에
C<4 * 5>이 먼저 평가받고 결과는 C<6 * 5 == 30> 이 아닌
C<2 + 20 == 22> 이 됩니다.
...
I<연산자의 결합성> 은 같은 연산자가 연속해서 나타날 경우에 무엇이 일어날지를 정의합니다. 
: 연산기가 좌측을 먼저 평가할 것인가 우측을 먼저 평가할 것인가 하는 것입니다.
예를들면, C<8 - 4 - 2>의 경우 빼기는 왼쪽결합이기에 펄은 계산식을 왼쪽에서 오른쪽으로 평가합니다.
C<8 - 4>이 먼저 평가되기때문에 C<8 - 2 = 6>이 아닌
C<4 - 2 == 2> 이 됩니다.
...
Perl 의 연산자에는 아래와같은 결합성과 우선순위(높은 우선순위부터 낮은 우선순위)가 있습니다.
C 로부터 가져온 연산자의 우선순위는 C 에서의 우선순위가 다소 이상해도, 그대로 쓰고 있습니다.
(이것은 C 를 사용해보신 분이 Perl 에 적응하기 쉽게 되어있습니다.)
몇 안되는 것을 빼면 모든 연산자는 스칼라값만 가지고, 배열값을 가지지 않습니다.
...
「항」은 Perl 에서 가장 우선순위가 높은 녀석입니다.
여기에는 변수, Quote와 Quote같은 연산자, 괄호로 감싼 임의의식 
인수를 괄호로 감싼 임의의함수가 포함됩니다.
실제로는 이 의미로 진짜 함수가 아닌 리스트 연산자와 함수같이 움직이는
단항연산자가 인수를 괄호로 감싸기때문에 그렇게 보입니다.
이것에 대해서는 L<perlfunc>에 기술하고 있습니다.
...
만약, 리스트연산자(print() 등등)나 단항연산자 (chdir() 등등) 의
이름의 뒤에 열린괄호('(')가 오는 경우에는 그 연산자와 괄호안의 인수는
보통 함수호출같이 무엇보다 높은 우선순위 처리됩니다.
...
괄호가 없는 경우에는, C<print>, C<sort> C<chmod> 같은 리스트 연산자의
우선순위는, 연산자의 왼쪽에서 하면 대단히 높고, 오른쪽에서 하면 대단히 낮게 보입니다.
예를 들면
...
위와 같은 경우 sort 의 오른쪽 콤마는 sort보다도 앞에서 평가되지만
(오른쪽에서 보면 soft의 우선순위기 낮음), 왼쪽의 콤마는 sort의 뒤에
평가됩니다(왼쪽에서 보면 sort 쪽이 우선순위가 높아짐)
바꿔 말하면, 리스트연산자는 자신의 뒤에 오는 인수를 모두 사용해 처리를 하지않고,
그 결과를 자신의 앞의 식에 대한 "항"인것 처럼 보이게 하는 것입니다.
물론, 괄호에는 주의해 주세요.
...
=end original
    ...
    # 다음은 print 를 실행하기 전에 exit 를 평가합니다.
    print($foo, exit);  # 이런 짓은 하고 싶지 않겠죠?
    print $foo, exit;   # 이것도 아니겠고
...
=end original
    ...
    # 다음은 exit 를 평가하기 전에 print를 실행합니다.
    (print $foo), exit; # 이런 것을 하려고 하셨죠?
    print($foo), exit;  # 아님 이거?。
    print ($foo), exit; # 이것도 OK。
...
의 동작을 한 번 보는 것만으로 판단하는 것은 어렵겠죠?
괄호는 C<print> 를 위해 평가되는 인수리스트를 감싸고 있습니다.
(C<$foo & 255> 라는 결과가 표시됩니다)。
그리고 C<print> 의 리턴값(보통은 1)에 1이 더해집니다. 
결과는 아래와 같습니다.
...
자신의 원했던 것을 적절히 실행하기 위해서는 아래와 같이써야됩니다.
...
자세한 내용은 L<Named Unary Operators> 을 참조해주세요.
...
이 외에는 "항"으로 해석되는 것으로는 C<do {}>나 C<eval {}>의 구성
서브루틴이나 메소드의 호출, 이름없는 구조인 C<[]>와 C<{}>가 있습니다.
...
뒤의 L<Quote and Quote-like Operators>와 L<"/I/O Operators">도 참조하세요.
...
C 와 C++ 과 마찬가지로, "C<< -> >>"는 가운데에 놓는 피참조 연산자입니다.
오른쪽이 C<[...]>, C<{...}>, C<(...)> 중 어떤 형태의 첨자이면 좌측은 배열,
해쉬, 서브루틴으로 하드 레퍼런스 아니면 심볼릭 레퍼런스 (혹은 기술적으로는 
해열 혹은 하드 레퍼런스가 대입가능하면 하드 레퍼런스를 가지는 장소) 가 아니면 안되니다.
L<perlreftut> 와 L<perlref> 를 참조하세요.
...
그렇지 않으면, 오른쪽은 메소드이름과 서브루틴의 리퍼런스를 가진 단순 스칼라변수로
왼쪽은 오브젝트 (bless 된 레퍼런스)  아니면 클래스 이름이 아니면 안됩니다.
L<perlobj> 를 참조하세요.
...
"++" 과 "--" 는 C의 경우와 같은 방식으로 동작합니다.
변수의 앞에 올때에는 값을 리턴하기 전에 변수를 1 증가 혹은 감소하고,
뒤에 올때에는, 값을 리턴한 뒤에 변수를 증가 혹은 감소합니다.
...
C 와 마찬가지로, Perl은 B<언제> 변수가 증가 혹은 감소되는 것에 대해서는 정의할 수 없습니다.
값이 리턴되기 전/후에 실행된다는 것 만 알 수 있습니다. 
이것은 같은 같은 구문인 변수를 2번 수정하면, 실행 혹은 미정의 상태가 되는 것을 의미합니다.
아래와 같은 것은 삼가해 주세요:
...
Perl 은 위와 같은 구문의 결과를 보장하지 않습니다.
...
증가연산자에는 조금 분위기가 다른 기능이 포함되어 있습니다.
숫자값이 들어간 변수나 숫자값의 문맥으로 사용된 변수를 증가시키는 경우에는
보통 증가 라는 의미로 동작하게 됩니다. 
그러나, 그 변수가 정의되고 계속 문자열의 문맥으로만 사용되지 않고
공백도 아닌, C</^[a-zA-Z]*[0-9]*\z/> 에 매치되는 값을 가지고 있을 때는 
각 문자의 범위를 보호하면서 글자수를 늘리고, 문자열로써의 증가를 수행합니다.
(매지컬 증가라고 불립니다.)
...
C<undef> 는 항상 숫자값으로 취급되고, 특히 증가되기전에는 C<0>로 바뀝니다.
(따라서, undef의 후증가값은 C<undef>가 아닌 C<0>가 됩니다).
...
이항연산자의 "**"는 지수연산자입니다.
이 연산자는 단항의 빼기에서 결합이 강한 연산자로
-2**4 는 (-2)**4 이 아니라 -(2**4) 로 해석됩니다.
(이것은 C 의 pow(3) 을 사용해 구축되어 있어서 내부적으로 double 로 동작합니다. 
...
단항연산자의 "!"는 논리부정을 처리합니다. 즉, "not"입니다.
이 연산자의 우선순위를 작게하려면 C<not>가 사용하면 됩니다.
X<!>
...
단항연산자의 "-"는 피연산자가 숫자값이면 산술부정합니다..
피연산자가  식별자라면, 빼기 기호(-)에 그 식별자를 이어 문자열이 반환됩니다.
이외에 피연산자의 최초의 문자가 더하기 혹은 빼기 일때에는
그 기호를 거꾸로 바꾼 문자열을 반환합니다.
이 규칙의 결과 -bareword 가 문자열 "-bareword"와 같은 값이 됩니다.
하지만, 문자열이 영어이외("+"와 "-"를 제외하고)의 문자로 시작되면
Perl 은 문자열을 수치로 변환하려고 하고, 그래서 산술부정이 샐행됩니다.
만약 문자열이 명확하게 숫자값으로 변환되지않을 때는, Perl은
B<Argument "the string" isn't numeric in negation (-) at ...> 라는
경고를 냅니다.
X<-> X<negation, arithmetic>
...
단항연산자의 "~"는 비트단위의 부정을 수행합니다. 즉, 1의 보수를 반환합니다.
예를들면, C<0666 & ~027> 는 0640 입니다.
(L<Integer Arithmetic> 과 L<Bitwise String Operators> 를 참조해주세요)
결과의 너비은 플랫폼에 의존하고 있으니 주의해주세요.
~0 는 32-bit 플랫폼에서 32비트의 너비이지만,
64-bit 플랫폼에서는 64비트의 너비이니
특정의 비트 너비를 가정하는 경우는, 남은 비트를 마스크하기위해 & 연산자를
사용하는 것을 잊지 않도록 주의 하세요.
...
단항연산자인 "+"는 예를 들어 문자열에 대해 사용할 경우에는 아무것도 하지 않습니다.
함수 이름에 이어 괄호가 붙은 식을 쓰는 경우에는, 함수의 인수리스트로 
해석되지 않기 위해서 사용할 수 있습니다.
(아래에 쓴 L<Terms and List Operators (Leftward)> 의 예를 참조해주세요)
X<+>
...
단항연산자인 "\" 는 뒤에 붙은 것에 대한 리퍼런스를 생성합니다.
L<perlreftut> 과 L<perlref> 를 참조해주세요.
이 사용법도 문자열 안에 백슬래쉬도 뒤에 오는 것 전개되는 것을 
막는 것이 가능하지만 동작을 혼동하지 말아주세요.
X<\> X<reference> X<backslash>
...
이항연산자인 "=~" 는, 스칼라식을 패턴매치로 묶습니다.
기본적으로 $_ 의 문자열을 검색하거나 변경하거나 하는 연산이 있습니다.
이 연산자는 그같은 연산을 다른 문자열에 대해 수행시키는 것입니다.
오른쪽 인수는 검색 패턴, 치환, 문자변환 중 어느 것입니다.
왼쪽 인수는 기본적인 $_ 대신에 검색, 치환, 문자변환의 대상이 됩니다.
스칼라콘텍스트에서 사용하면 반환값은 일반적으로 연산의 결과가 성공했는가 실패했는 가 입니다.
리스트콘텍스트에서의 행동은 연산자에 의존합니다.
자세한 내용은 L</"Regexp Quote-Like Operators"> 를, 
이 연산자들을 사용한 예제에 대해서는 L<perlretut> 를 참조하세요。
...
오른쪽인수가 검색 패턴, 치환, 문자변환이 아닌 식이라면 
그것은 실행할 때에 결정되는 검색패턴으로 해석됩니다.
이것은 내용이 2번 전개되는 것을 의미하는 것임으로 주의해주세
즉:
...
은 바르지 않습니다. 정규표현엔진은 최종적으로 패턴 C<\>를
컴파일하려고해서, 이것은 문법에러로 생각하기 때문입니다.
...
이항연산자인 "!~"는 반환값이 논리부정되는 것을 제외하고는 "=~"와 같습니다.
...
이항연산자인 "%" 는 2개의 숫자값의 나머지를 반환합니다.
C<$a> 와 C<$b> 의 2개의 정수의 피연산자를 취합니다.
C<$b> 가 높은 경우 C<$a % $b> 는 C<$a> 에서 C<$a> 를 넘지않는
최대의 C<$b> 의 배수를 끌어온 값(?)입니다. 
C<$b> 가 낮은 경우 C<$a % $b> 는 C<$a> 에서 C<$a> 보다 낮지 않은 
최소의 C<$b> 배수를 끌어온 값입니다.(따라서 결과는 0 이하)
If the operands
C<$a> and C<$b> are floating point values and the absolute value of
C<$b> (that is C<abs($b)>) is less than C<(UV_MAX + 1)>, only
the integer portion of C<$a> and C<$b> will be used in the operation
...
C<use integer> 가 스코프 안에 있는 경우,
"%" 는 C 컴파일러로 구축된 나머지연산자를 사용합니다. 
이 연산자는 피연산자가 낮은 경우의 움직임이 불확실하지만 보다 빠르게 실행됩니다.
X<%> X<remainder> X<modulus> X<mod>
(TBT)
...
이항연산자인 "x" 는 반복반환연산자입니다.
스칼라 컨텍스트 혹은 왼쪽의 값이 괄호로 감싸진 경우에는
왼쪽 피연산자를 오른쪽 피연산자에 표시 횟수만 반복해 얻은 값으로 구성된
문자열을 리턴합니다.
리스트 컨텐스트에서는 왼쪽 피연산자가 괄호 감싸진 경우 혹은 , 　C<qw/STRING>의
형태의 리스트의 경우 리스트를 반복한 값이 반환됩니다.
오른쪽 피연산자가 0 혹은 낮은 값일 경우, 콘텍스트에 의해 빈 문자열 혹은 
빈 리스트를 반환합니다.
...
이항연산자인 "<<" 는 왼쪽 인수의 값을 오른쪽 인수가 나타내는 비트의 수만큼
왼쪽으로 시프트한 값을 반환합니다.
인수는 정수가 아니면 안됩니다.
