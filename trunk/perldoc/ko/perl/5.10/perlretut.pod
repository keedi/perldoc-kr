...
매치되지 않는다. 세번째 정규식은 매치된다. C<$>은 C<keeper>을 오직
문자열의 마지막에서 매치되기를 강제했다.
...
매치되어야한다. 예를 들면, 정규식은 문자열 전체와 매치한다. 잘생각해보자.
...
C<$> 모두를 사용하는것은 문자열 전부와 매치를 강요하므로, so it gives you
complete control over which strings match and which don't.  Suppose you are
looking for a fellow named bert, off in a string by himself:
