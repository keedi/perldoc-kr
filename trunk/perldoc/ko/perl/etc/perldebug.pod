...
C<maxdepth> 가 지정되어진 경우, 그것을 변수 I<N> 이 아니면 안됩니다;
...
다음. 다음의의 실행문의 맨앞에 도달하기 까지, 서브루틴<BLAH>
...
regex 에 일치하는(또는 일치하지않는) 서브루틴 이름을 리스트업합니다.
...
condition 이 잠이 되었을 때에만 브레이크 포인트가 작동합니다.
...
I<line> 이 생략되면, 지금 바로 실행하고자하는 줄에 설정되어있는
액션이 삭제됩니다.
...
인수리스트가 <BLAH> 했을 때ㅣ 길이는 다음의 옵션의 <BLAH> 가 됩니다.
